...
%   Modified May 2007:
%   - change in call to mkdir for compatibility with Octave
...
if start 
   ...
   % manage filters for operator equalisation:
   if params.opeq && ~strcmp(params.opeq_target,'none')
       params.filters{end+1}=params.opeqtype;
   end
   
   if params.gde
       params.filters{end+1}='bestfitness';
   end
   ...
   if isempty(params.gengap)
    ...
    % manage RST parameter rst_rss:
   if params.rst_rss<=0 % if rst_rss is negative
         % correct possible error:
         params.rst_rss=1;
         if ~strcmp(params.output,'silent')
            displaystatus('\n   (increasing parameter ''rst_rss'' to %d)\n',params.rst_rss);
         end
   end
   if params.rst_rss>=1 && mod(params.rst_rss,1)~=0 % if rst_rss is >=1 and not integer 
         % correct possible error:
         params.rst_rss=round(params.rst_rss);
         if ~strcmp(params.output,'silent')
            displaystatus('\n   (rounding parameter ''rst_rss'' to %d)\n',params.rst_rss);
         end
   end
...
if ~strcmp(params.savetofile,'never')
    ...
    % (ignore status (ans), just look at msg for compatibility with Octave)
    [ans,msg]=mkdir([parentdir dirname]);
    if ~isempty(msg)
        error(['CHECKVARSPARAMS: ' msg]);
    end
...
if or(params.usetestdata,params.rst)
    params.keepevalssize=0;
    % only use cache when you have "normal" training data, and no test data
end
