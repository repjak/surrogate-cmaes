...
if start
   ...
   % initialize "opeq" variable (operator equalisation):
   if params.opeq
      state.opeq.target_occupancy=[];
      state.opeq.current_occupancy=[];
      state.opeq.exists=[];
      state.opeq.full=[];
      state.opeq.fitnesses={};
      state.opeq.bestfitness=[];
      state.opeq.worstfitness=[];
      state.opeq.rejected=[];
      state.opeq.rejectedsizes=[];
      state.opeq.history.target_occupancy=[];
      state.opeq.history.current_occupancy=[];
      state.opeq.history.rejected=[];
      state.opeq.history.rejectedsizes=[];
   end
   
   % initialize rejections variable to count non-opeq rejections:
   state.rejected=[];
   state.historyrejected=[];
