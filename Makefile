...
OUT = $(FNAME)
SRC = exp/$(FNAME).m
OTHERS = exp/bbob_test_01.m exp/metacentrum_task_matlab.m exp/opt_* src/surrogateManager.m src/model/* src/cmaes/*

# $(OUT):	$(SRC) $(OTHERS)
# 	$(MATLAB_COMPILER) -m $(MC_FLAGS) $(MC_INCLUDE) -o $@ $<
#	cp -p $@ $(DESTDIR)
$(DESTDIR)/$(OUT):	$(SRC) $(OTHERS)
	$(MATLAB_COMPILER) -m $(MC_FLAGS) $(MC_INCLUDE) -o $(OUT) $<
	mv $(OUT) $(DESTDIR)
