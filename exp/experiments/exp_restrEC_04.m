exp_id = 'exp_restrEC_04';
exp_description = 'Surrogate CMA-ES model using restricted individual EC with sd2 criterion and GPs with (a) f-value normalization (b) non-normalized; GPs with improved performance';
...
sgParamDef(6).values = { 10 }; % {8, 16}; % {2, 3, 4, 6, 8};
...
modelOptsValues = struct( ...
  ...
  'minLeaf', 2, ...
  'inputFraction', 1);
mov1 = modelOptsValues;
mov2 = modelOptsValues;
mov1.normalizeY = true;
mov2.normalizeY = false;
sgParamDef(12).values = { mov1, mov2 };
sgParamDef(13).name  = 'evoControlRestrictedParam';
sgParamDef(13).values = {0.1, 0.2};
